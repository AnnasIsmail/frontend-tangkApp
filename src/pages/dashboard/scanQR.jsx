...
const ScanQR = () => {
  ...
  const token = 'YOUR_AUTH_TOKEN'; // Masukkan token Anda
  const roleNow = 'YOUR_ROLE'; // Masukkan role Anda
  ...
  return (
    <div>
      <h1>Scan QR Code</h1>
      <ScanQRCode
        onScan={(id) => fetchBerkas(id)} // Memanggil fetchBerkas dengan ID hasil scan
      />
      <GenerateQRCode />
      {showDetailPopUp && (
        <DetailModal
